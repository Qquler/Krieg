...
public class Necron_Controller : MonoBehaviour
{
    ...
    void Awake()
    {
       th = this.GetComponent<EnemyController>();
        if (th.fullHP != fullHP)
        {
            th.fullHP = fullHP;
        }
    ...
    {
        if (th.GetcurHP() + deltaHeal > fullHP)
        {
            th.ChangecurHP(fullHP);
        }
        else
        {
            th.ChangecurHP(deltaHeal + th.GetcurHP());
        }
